...
// 백준 1260 - DFS 와 BFS - 인접 행렬 사용
public class Graph01_array {
    ...
    static void bfs(int start) {
        ...
        que.add(start);
        visit[start] = true; // start 를 갈 수 있다가 표시하기 (중요!!!)
        ...
        while (!que.isEmpty()) { // 더 확인할 점이 없다면 정지
            ...
            for (int y = 1; y <= N; y++) {
                if (adj[x][y] == 0) continue;
                if(visit[y]) continue; // x 에서 y를 갈 수 있지만, 이미 탐색한 점이면 무시

                // y 를 갈 수 있으니까 que에 추가하고, visit 처리 하기!
                que.add(y);
                visit[y] = true;
            }
        }
